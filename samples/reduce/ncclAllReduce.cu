...
%
%  Purpose:
%   Implements sample REDUCE code using the package NCCL(ncclAllReduce).
...
%
%  How to Compile:
%   nvcc ncclAllReduce.cu -o ncclAllReduce -lnccl  
%
%  Execute: 
%   ./ncclAllReduce <size problem>                              
%   ./ncclAllReduce     4
%****************************************************************************80*/
...
int main(int argc, char* argv[]) {
  ...
  for(int g = 0; g < nGPUs; g++) {
      cudaSetDevice(DeviceList[g]);            
      printf("\n This is device %d\n", g);
      Dev_dot <<< 1, data_size >>> (Sy_d_data[g], Sx_d_data[g], data_size); /*Call the CUDA Kernel: Calculate dot product*/
